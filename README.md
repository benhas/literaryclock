...
## Quote data files and special minutes

* The main quote CSV used for image generation is `quote to image/litclock_annotated_br2.csv`, a pipe‑delimited file with the columns: `time|timestring|quote|title|author|extra`.
* Normal rows use a real **24‑hour minute** in the `time` column (e.g. `07:32`, `18:05`). The `timestring` is the exact fragment of the quote that denotes this time (e.g. `twenty‑eight minutes past one`, `7:32 p.m.`).
* Rows with `time` set to `99:99` are **meta / time‑agnostic quotes** (about time itself). They are not used for any specific minute of the day but can be used as fallbacks or for debugging.
* Rows with `time` set to `88:88` are **battery / low‑energy quotes**. These are short exhaustion / “low fuel” snippets intended for display when the Kindle battery is low (for example, <20%) instead of a normal time quote. They are not tied to a real clock minute.
* The helper file `missing-minutes-log.json` (at the repo root) is generated by tools to track which 24‑hour minutes still have no quote and which minutes were filled from external literary‑clock data. It is informational only; the clock itself reads solely from `litclock_annotated_br2.csv`.
